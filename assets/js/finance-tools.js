...
function finMoneySet(el,value){if(!el)return;if(value===''){el.value='';return}const n=Number(value||0);el.value=Number.isFinite(n)?n.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2}):''}
function finMoneyEdit(el){if(!el||!el.value.trim())return;const n=finParseMoney(el.value);el.value=Number.isFinite(n)?n.toFixed(2):''}
function finNormalizeMoneyTyping(raw){raw=String(raw??'').replace(/S\/?/gi,'').replace(/\s/g,'');let out='',decimal=false,decimals=0;for(const ch of raw){if(/\d/.test(ch)){if(decimal){if(decimals>=2)continue;decimals++}out+=ch}else if((ch==='.'||ch===',')&&!decimal){if(!out)out='0';out+='.';decimal=true}}return out}
function finSanitizeMoneyTyping(el){if(!el||el.dataset.finMoneyFormatting==='1')return;const raw=el.value,pos=el.selectionStart??raw.length,before=finNormalizeMoneyTyping(raw.slice(0,pos)),clean=finNormalizeMoneyTyping(raw);if(raw===clean)return;el.dataset.finMoneyFormatting='1';el.value=clean;const next=Math.min(clean.length,before.length);requestAnimationFrame(()=>{try{el.setSelectionRange(next,next)}catch{}delete el.dataset.finMoneyFormatting})}
function finBindMoney(root=document){root.querySelectorAll?.('[data-money-input]').forEach(el=>{if(el.dataset.finMoneyBound)return;el.dataset.finMoneyBound='1';el.addEventListener('focus',()=>finMoneyEdit(el));el.addEventListener('input',()=>finSanitizeMoneyTyping(el));el.addEventListener('paste',e=>{const text=e.clipboardData?.getData('text');if(text==null)return;e.preventDefault();const n=finParseMoney(text);el.value=Number.isFinite(n)?n.toFixed(2):'';requestAnimationFrame(()=>{try{el.setSelectionRange(el.value.length,el.value.length)}catch{}})});el.addEventListener('blur',()=>{if(el.value.trim())finMoneySet(el,finParseMoney(el.value))})})}

function initFinanceTools({root=document}={}) {
 const pageAbort=new AbortController();
 const api=f=>`${APP.apiBase}/${f}`; const $=s=>document.querySelector(s);
 async function req(file,data){const headers=window.MiDinero?.clientHeaders({'Content-Type':'application/json','X-CSRF-Token':APP.csrf})||{'Content-Type':'application/json','X-CSRF-Token':APP.csrf};const r=await fetch(api(file),{method:'POST',headers,body:JSON.stringify(data),signal:pageAbort.signal}),j=await r.json();if(!r.ok||j.ok===false)throw new Error(j.message||'Error');return j;}
 ...
 const newAccount=$('#newAccountForm');if(newAccount)newAccount.onsubmit=async e=>{e.preventDefault();try{await req('account_save.php',Object.fromEntries(new FormData(e.target).entries()));await window.MiDinero.softRefresh({preserveScroll:true})}catch(err){finNotice(err.message,'error')}};
 const transfer=$('#transferForm');if(transfer)transfer.onsubmit=async e=>{e.preventDefault();try{await req('account_transfer.php',Object.fromEntries(new FormData(e.target).entries()));await window.MiDinero.softRefresh({preserveScroll:true})}catch(err){finNotice(err.message,'error')}};
 const newFund=$('#newFundForm');if(newFund)newFund.onsubmit=async e=>{e.preventDefault();try{await req('fund_save.php',Object.fromEntries(new FormData(e.target).entries()));await window.MiDinero.softRefresh({preserveScroll:true})}catch(err){finNotice(err.message,'error')}};
 ...
 document.addEventListener('click',e=>{if(!e.target.closest('.fund-menu-wrap'))closeFundMenus()},{signal:pageAbort.signal});
 document.addEventListener('keydown',e=>{if(e.key==='Escape')closeFundMenus()},{signal:pageAbort.signal});
