...
$inc=db()->prepare("SELECT t.id,t.amount,t.occurred_at,COALESCE(co.name,c.name) name,t.amount-COALESCE((SELECT SUM(fa.amount) FROM fund_allocations fa WHERE fa.user_id=t.user_id AND fa.source_transaction_id=t.id AND fa.amount>0 AND fa.voided_at IS NULL),0) remaining FROM transactions t JOIN categories c ON c.id=t.category_id LEFT JOIN concepts co ON co.id=t.concept_id WHERE t.user_id=? AND t.voided_at IS NULL AND t.type='income' HAVING remaining>0 ORDER BY t.occurred_at DESC LIMIT 12");$inc->execute([$uid]);$incomes=$inc->fetchAll();
