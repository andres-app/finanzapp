...
try {
    ...
    $st = $pdo->prepare("SELECT mp.*,r.category_id,r.concept_id,r.name,r.icon,r.fund_id recurring_fund_id
        ...
        WHERE mp.id=? AND mp.user_id=? AND mp.status='pending' FOR UPDATE");
    $st->execute([$id,$uid]);
    $p = $st->fetch();
    if (!$p) throw new DomainException('Pago no encontrado o ya fue registrado.');
    ...
    $referenceAmount=(float)$p['amount'];
    ...
    $tx->execute([$uid,actual_user_id(),'expense',$p['category_id'],$p['concept_id'],$accountId,$fundId,$amount,$occurredAt,'Pago mensual: '.$p['name'],$paymentMethod]);
    $txId=(int)$pdo->lastInsertId();

    // Cerrar la obligación por identidad financiera (pago fijo + período), no solo
    // por el id visual. Esto evita que duplicados históricos de monthly_payments
    // dejen el compromiso apareciendo como pendiente después de haber pagado.
    ...
    $up->execute([$occurredAt,$txId,$uid,(int)$p['recurring_id'],(string)$p['period']]);
    if ($up->rowCount() < 1) throw new RuntimeException('El pago cambió mientras se procesaba.');

    $pdo->commit();
    emit_event($uid,'payment_paid',['id'=>$id,'transaction_id'=>$txId,'amount'=>$amount,'reference_amount'=>$referenceAmount]);
    $user=current_user();
    Mailer::transaction($user,['type'=>'expense','amount'=>$amount,'occurred_at'=>$occurredAt,'description'=>'Pago mensual: '.$p['name'],'concept_name'=>$p['name'],'category_name'=>'Obligación']);
    json_response(['ok'=>true,'amount'=>$amount,'reference_amount'=>$referenceAmount,'transaction_id'=>$txId]);
